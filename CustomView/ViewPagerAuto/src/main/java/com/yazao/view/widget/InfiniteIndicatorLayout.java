...
import com.yazao.view.MResource;
import com.yazao.view.widget.indicator.PageIndicator;
import com.yazao.view.widget.indicator.RecyleAdapter;
import com.yazao.view.widget.salvage.RecyclingPagerAdapter;
import com.yazao.view.widget.slideview.BaseSliderView;

import java.lang.reflect.Field;

/**
 * Author:  MoonLife
 * Time: 2015/12/15 17:41
...
//    private boolean isBorderAnimation = true;
	...
	public InfiniteIndicatorLayout(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		try {
			final TypedArray attributes = context.obtainStyledAttributes(attrs, MResource.getIdsByName(mContext, "styleable", "ag_InfiniteIndicatorLayout"),0,0);
			int indicatorType = attributes.getInt(MResource.getIdByName(mContext, "styleable", "ag_InfiniteIndicatorLayout_ag_indicator_type"),IndicatorType.Default.ordinal());

			if(indicatorType == 0)
				LayoutInflater.from(context).inflate(MResource.getIdByName(mContext, "layout", "ag_layout_default_indicator"), this, true);
			else if(indicatorType == 1)
				LayoutInflater.from(context).inflate(MResource.getIdByName(mContext, "layout", "ag_layout_anim_circle_indicator"), this, true);
			else
				LayoutInflater.from(context).inflate(MResource.getIdByName(mContext, "layout", "ag_layout_anim_line_indicator"), this, true);
		} catch (Exception e) {
			e.printStackTrace();
		}

		mViewPager = (ViewPager) findViewById(MResource.getIdByName(mContext, "id", "ag_view_pager"));
	...
	public enum IndicatorPosition {
		Center("Center_Bottom", MResource.getIdByName(mContext, "id", "ag_default_center_indicator")),
		Center_Bottom("Center_Bottom", MResource.getIdByName(mContext, "id", "ag_default_center_bottom_indicator")),
		Right_Bottom("Right_Bottom", MResource.getIdByName(mContext, "id", "ag_default_bottom_right_indicator")),
		Left_Bottom("Left_Bottom", MResource.getIdByName(mContext, "id", "ag_default_bottom_left_indicator")),
		Center_Top("Center_Top", MResource.getIdByName(mContext, "id", "ag_default_center_top_indicator")),
		Right_Top("Right_Top", MResource.getIdByName(mContext, "id", "ag_default_center_top_right_indicator")),
		Left_Top("Left_Top", MResource.getIdByName(mContext, "id", "ag_default_center_bottom_indicator"));
